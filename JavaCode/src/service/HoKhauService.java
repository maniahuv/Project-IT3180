...
public class HoKhauService {
	...
	public static boolean taoHoKhauMoi(HoKhau t) {
		if (kiemTraThongTin(t)) {
			if (HoKhauDao.instance.insert(t) > 0) {
				LichSuService.ghiNhanLichSu("Them ho khau", "", "");
				return true;
			}
		}
		return false;
	...
	public static boolean xoaHoKhau(String id) {
		if (HoKhauDao.instance.delete(id) > 0) {
			LichSuService.ghiNhanLichSu("Xoa ho khau", "", "");
			return true;
		}
		return false;
	}

	public static boolean luuBienDong(HoKhau t) {
		if (kiemTraThongTin(t)) {
			if (HoKhauDao.instance.update(t.getMaHoKhau(), new String[] { "SoCanHo", "DienTich", "SoNguoi", "ChuHo" },
				...
				LichSuService.ghiNhanLichSu("Cap nhat thong tin ho khau", "", "");
				return true;
			}
		}
		return false;
